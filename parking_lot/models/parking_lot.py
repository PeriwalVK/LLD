...
class ParkingLot:
    def __init__(self):
        ...
        self.parking_spots: Dict[VehicleType, List[ParkingSpot]] = {
            VehicleType.CAR: [
                ParkingSpot(f"{VehicleType.CAR.value}_{i}", VehicleType.CAR)
                for i in range(DefaultValues.CAR_SPOT_COUNT)
            ],
            VehicleType.BIKE: [
                ParkingSpot(f"{VehicleType.BIKE.value}_{i}", VehicleType.BIKE)
                for i in range(DefaultValues.BIKE_SPOT_COUNT)
            ],
            VehicleType.TRUCK: [
                ParkingSpot(f"{VehicleType.TRUCK.value}_{i}", VehicleType.TRUCK)
                for i in range(DefaultValues.TRUCK_SPOT_COUNT)
            ],
        }
    ...
    def park_vehicle(
        ...
        try:
            print(
                f"[ParkingLot]: {vehicle.vehicle_type.value} {vehicle.vehicle_number} trying to enter parking lot..."
            )
            parking_spot: ParkingSpot = parking_strategy.fetch_parking_spot(
                self.parking_spots, vehicle.vehicle_type
            )

            ticket = ParkingTicket(vehicle, parking_spot, fare_strategy)
            self._vehicle_enters(ticket, entry_epoch)
            print(
                f"[ParkingLot]: {vehicle.vehicle_type.value} {vehicle.vehicle_number} has been parked at {parking_spot.spot_id}. Ticket Number: {ticket.id}\n"
            )
            return ticket
        except NoParkingSpotAvailableException as e:
            print(
                f"[ParkingLot]: No Parking Spot Available for {vehicle.vehicle_type.value} {vehicle.vehicle_number}...\n"
            )

    def _increase_earnings_by(self, fare: int):
        self.total_earnings += fare
        print(f"[ParkingLot]: Total Earnings: ${self.total_earnings}")

    def _make_payment(self, fare: float, payment_strategy: PaymentStrategy):
        payment_strategy.make_payment(fare)

    def unpark_vehicle(
        self, ticket: ParkingTicket, exit_epoch: int, payment_strategy: PaymentStrategy
    ) -> None:
        print(
            f"[ParkingLot]: {ticket.vehicle.vehicle_type.value} {ticket.vehicle.vehicle_number} is trying to exit the parking lot..."
        )
        fare = ticket.calculate_fare(exit_epoch)
        self._make_payment(fare, payment_strategy)
        self._vehicle_exits(ticket, exit_epoch)
        self._increase_earnings_by(fare)

        print(
            f"[ParkingLot]: {ticket.vehicle.vehicle_type.value} {ticket.vehicle.vehicle_number} Exited the Parking Lot.\n"
        )
